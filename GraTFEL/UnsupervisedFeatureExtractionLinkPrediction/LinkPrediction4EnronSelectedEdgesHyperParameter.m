...
for s=0:stamps
    s
    %s=0;
    file=strcat('../../Graphs/EnronAdjDelta',num2str(s),'.txt');
    ...
    load EnronAdjTruth2.txt
    Truth=EnronAdjTruth2;
...
save('LinkPrediction3EnronLabellFeatureHop3','Data','stamps','-v7.3')
end
clear all
%stamps=9;
mkdir('Result','LinkPrediction3Enron')
load LinkPrediction3EnronLabellFeatureHop3
...
fn=strcat('Result/LinkPrediction3Enron/ResultSA_HL_Study','_noUL','.mat');
save(fn)
...
%         [~,~,~,result(i,2),result(i,1),~,result(i,3)]=Classifier(TrainPC, TrainLabel, TestPC, TestLabel,n,et,'Result/Enron/15SA_SVM',p);
%         [~,~,~,resultAdaBoostM1(i,2),resultAdaBoostM1(i,1),~,resultAdaBoostM1(i,3)]=ClassifierAdaBoostM1(TrainPC, TrainLabel, TestPC, TestLabel,n,et,'Result/Enron/15SA_SVM',p);
%         [~,~,~,resultRobustBoost(i,2),resultRobustBoost(i,1),~,resultRobustBoost(i,3)]=ClassifierRobustBoost(TrainPC, TrainLabel, TestPC, TestLabel,n,et,'Result/Enron/15SA_SVM',p);        
    end

    fn=strcat('Result/LinkPrediction3Enron/ResultSA_HL_Study',num2str(HL),'.mat');
    save(fn)
end
%r=zeros(903,1);
%for i=3:903
