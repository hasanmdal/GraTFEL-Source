...
for s=0:stamps
    s
    %s=0;
    file=strcat('../../Graphs/DBLP2AdjDelta',num2str(s),'.txt');
    ...
    load DBLP2AdjTruth2.txt
    Truth=DBLP2AdjTruth2;
...
save('LinkPrediction3DBLP2LabellFeatureHop3','Data','stamps','-v7.3')
end

clear all
mkdir('Result','LinkPrediction3DBLP2')
load LinkPrediction3DBLP2LabellFeatureHop3
...
%Test=cat(2, Data{9}(:,4:end),Data{9}(:,3));
...
fn=strcat('Result/LinkPrediction3DBLP2/ResultSA_HL_Study','_noUL','.mat');
save(fn)
...
%         
%         [~,~,~,result(i,2),result(i,1),~,result(i,3)]=Classifier(TrainPC, TrainLabel, TestPC, TestLabel,n,et,'Result/DBLP2/15SA_SVM',p);
%         [~,~,~,resultAdaBoostM1(i,2),resultAdaBoostM1(i,1),~,resultAdaBoostM1(i,3)]=ClassifierAdaBoostM1(TrainPC, TrainLabel, TestPC, TestLabel,n,et,'Result/DBLP2/15SA_SVM',p);
%         [~,~,~,resultRobustBoost(i,2),resultRobustBoost(i,1),~,resultRobustBoost(i,3)]=ClassifierRobustBoost(TrainPC, TrainLabel, TestPC, TestLabel,n,et,'Result/DBLP2/15SA_SVM',p);        
    end

    fn=strcat('Result/LinkPrediction3DBLP2/ResultSA_HL_Study',num2str(HL),'.mat');
    save(fn)
end
%r=zeros(903,1);
%for i=3:903
